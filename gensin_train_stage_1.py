...
from src.dataset.gensin_image import GensinImgDataset
...
process_group_kwargs = InitProcessGroupKwargs(timeout=timedelta(seconds=70800))  # 1.5 hours
...
def log_validation(
    ...
    denoising_unet = copy.deepcopy(ori_net.denoising_unet).to(dtype=torch.float32)
    ...
    ref_image_paths = [
        "/data2/zjx/dataset/gensin_pv/test_video/Shuishen_img/00001270.jpg",
        "/data2/zjx/dataset/gensin_pv/test_video/Naweiya_img/00000530.jpg",
        "/data2/zjx/dataset/gensin_pv/raw_img/00023710.jpg",
        "/data2/zjx/dataset/gensin_pv/raw_img/00034998.jpg",
    ]
    ...
    for ref_image_path in ref_image_paths:
        ...
        pil_images.append({"name": f"{ref_name}", "img": canvas})
...
def main(cfg):
    ...
    train_dataset = GensinImgDataset(
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        if (epoch + 1) % cfg.save_model_epoch_interval == 0 and accelerator.is_main_process:
            ...
            # reference_unet_cp = copy.deepcopy(unwrap_net.reference_unet).to(dtype=torch.float32, device='cpu')
            # denoising_unet_cp = copy.deepcopy(unwrap_net.denoising_unet).to(dtype=torch.float32, device='cpu')
            save_checkpoint(
        ...
        # debug saving checkpoint when using deepspeed
        # if (epoch + 1) % cfg.save_model_epoch_interval == 0 and accelerator.is_main_process:
        #     save_checkpoint(
        #         net.reference_unet,
        #         save_dir,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/training/gensin_stage1.yaml")
